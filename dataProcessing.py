            ...
            if dataset_addr[-3:] != 'npy':
                ...
                try:
                    ...
                    while (cap.isOpened()):
                        ...
                        ret, frame = cap.read()
                        if ret == True:
                            # Store frame to save as npy later
                            all_images_list.append(frame)

                            # Save frame as png file
                            # Pad frame counter to ease sorting
                            frame_counter_padded = str(frame_counter).zfill(7)
                            image_addr = os.path.join(dataset_addr, f'{frame_counter_padded}.png')
                            cv2.imwrite(image_addr, frame)
